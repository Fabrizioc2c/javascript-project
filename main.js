...
function getWeather(latitude, longitude, _inputValue) {
...
form.addEventListener("submit", (e) => {
  ...
  //check if there's the city     g global 
  //This is sparta!'.replace(/t/,'X');
//result: 'This is sparXa!' 
  if (weatherData.length > 0) {
    const existingWeatherInfo = weatherData.find((data) => {
        console.log(inputVal.replace(/ g/, ""),`${data.typedCityName},${data.country}`)
      if (
        data.city === inputVal ||
        data.typedCityName === inputVal ||
        inputVal.replace(/ g/, "") === `${data.city},${data.country}` ||
        inputVal.replace(/ g/, "") === `${data.typedCityName},${data.country}`
      )
        return true;
    });
    if (existingWeatherInfo) {
      msg.textContent = `You already know the weather for ${existingWeatherInfo.city} ...otherwise be more specific by providing the country code as well 😉`;
      form.reset();
      input.focus();
      return;
    }
  ...
  //             content = el.querySelector(".city-name span").textContent.toLowerCase();
  //         }
  //         return content.toLowerCase().trim() == inputVal.toLowerCase().trim();
  //     });
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      weatherData.push({
        typedCityName: cityName,
        city: name.toLowerCase(),
        country: sys.country.toLowerCase(),
      });
